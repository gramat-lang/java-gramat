...
public class MachineFormatter {
    ...
    public static String toString(MachineProgram machineProgram) {
        var buffer = new StringBuilder();
        var formatter = new MachineFormatter();

        formatter.writeComment(buffer, "main");
        formatter.writeMachine(buffer, machineProgram.main);
        formatter.writeNewLine(buffer);

        for (var entry : machineProgram.dependencies.entrySet()) {
            formatter.writeComment(buffer, entry.getKey());
            formatter.writeMachine(buffer, entry.getValue());
            formatter.writeNewLine(buffer);
        }
    ...
    public static String toString(Machine machine) {
        ...
        new MachineFormatter().writeMachine(buffer, machine);
    ...
    private String writeLabel(Link link) {
        ...
        writeActions(label, link.getBeginOperations(), "", "\n");
        ...
        writeActions(label, link.getEndOperations(), "\n", "");
